...
class ConditionEncoder(nn.Module):
    def __init__(self, 
        ...
        self.encoder = TemporalConvNet(input_dim=input_dim, 
        ...
        for _ in range(dec_length):
            self.decoder_list.append(LinearDecoder(dec_dim=input_dim-1, 
                                             hidden_dim=hidden_size, 
                                             enc_outdim=channels_dim[-1],
                                             z_dim=z_dim))
        

    def forward(self, source, time_cov):
        """
        输出的长度为需要预测的长度
        """
        enc_len = source.shape[1]
        enc_data = torch.cat([source, time_cov[:, :enc_len, :]], dim=-1)
        dec_data = time_cov[:, enc_len:, :]
        h = self.encoder(enc_data.permute(0, 2, 1)).permute(0, 2, 1)
        mean_p_list, logs_p_list = [], []
        for i in range(self.dec_length):
            mean_p_1step, logs_p_1step =  self.decoder_list[i](dec_data[:,i,:], h[:,-1,:])
            mean_p_list.append(mean_p_1step)
            logs_p_list.append(logs_p_1step)
        mean_p = torch.stack(mean_p_list, 1)
        logs_p = torch.stack(logs_p_list, 1)
        # mean_p, logs_p = self.decoder(dec_data, h[:,-1,:])
        

        return mean_p, logs_p
    
class LinearDecoder(nn.Module):
    def __init__(self, dec_dim, hidden_dim, enc_outdim, z_dim) -> None:
        super().__init__()
        self.z_dim = z_dim
        self.dense_net = nn.Sequential(nn.Linear(dec_dim, hidden_dim),
        ...
        self.is_dowmsample = False

        if enc_outdim != hidden_dim:
            self.is_dowmsample = True
            self.dowmsample = nn.Linear(enc_outdim, hidden_dim)

        self.lastlayer = nn.Linear(hidden_dim, z_dim*2) # 预测99个分位数

        

    def forward(self, x, h):
        if self.is_dowmsample:
            z = self.relu(self.dense_net(x) + self.dowmsample(h))
        else:
            z = self.relu(self.dense_net(x) + h)

        status = F.elu(self.lastlayer(z)) 

        mean_p, logs_p = torch.split(status, [self.z_dim]*2, -1)
...
class PosteriorEncoder(nn.Module):
    ...
    def forward(self, source, target, cov):
        ...
        z_q = self.net(x.permute(0,2,1)).permute(0,2,1)
...
class FCVAE_C(nn.Module):
    ...
    def loss(self, source, target, cov):
        ...
        # kl = kl_loss(z_p, logs_q, mean_p, logs_p, logdet_sum)
        return l1 + lh
...
def kl_loss(z_p, logs_q, m_p, logs_p, logdet_sum):
    """
    z_p, logs_q: [b, h, t_t]
    m_p, logs_p: [b, h, t_t]
    """
